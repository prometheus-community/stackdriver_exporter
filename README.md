...
| `monitoring.max-concurrency`        | No       | `0`                       | Maximum number of concurrent Monitoring API time series requests across all projects. `0` means unbounded. Set this when `google.projects.filter` or a long `google.project-ids` list resolves to many projects, to bound memory instead of relying on `GOMAXPROCS`/`GOMEMLIMIT` tuning. |
