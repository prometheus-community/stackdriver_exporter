...
| `google.projects.filter`            | No       |                           | GCloud projects filter expression. See more [here](https://cloud.google.com/sdk/gcloud/reference/projects/list). Resolving and refreshing this filter uses the same `stackdriver.http-timeout`, `stackdriver.max-retries`, and `stackdriver.retry-statuses` settings as the Monitoring API calls. |
